...
// *****Section J9.1 - Code Reference Point – Start******

// === PLAYER SHIP LOCATION TRACKING ===
// This section tracks the player's exact position on the map.
function trackPlayerLocation() {
  return {
    x: playerShip.x.toFixed(5), // Pinpoint accuracy to 5 decimal places
    y: playerShip.y.toFixed(5)
  };
}

// *****Section J9.1 - Code Reference Point – Finish******

// *****Section J9.2 - Code Reference Point – Start******

// === LASER TARGETING SYSTEM ===
// Stores the player's location and the last clicked target location.
let laserTarget = null;

// Track the player's ship location
function trackPlayerLocation() {
  return {
    x: playerShip.x,
    y: playerShip.y
...
// Track where the player clicks on the canvas
canvas.addEventListener("click", (event) => {
  ...
  // Convert the click position to game-world coordinates
  const targetX = playerShip.x + (clickX - width / 2);
  const targetY = playerShip.y + (clickY - height / 2);

  // Store the target position
  laserTarget = { x: targetX, y: targetY };

  console.log(`Laser Target Set: (${laserTarget.x}, ${laserTarget.y})`);
});

// Function to get the current laser target
function getLaserTarget() {
  return laserTarget;
}

// *****Section J9.2 - Code Reference Point – Finish******
...
// *****Section J12.1 - Code Reference Point – Start******

// === MOUSE CLICK EVENT TO FIRE LASER ===
canvas.addEventListener("click", (event) => {
  ...
  // Convert the click position to game coordinates
  // (Assuming the canvas is centered on the player)
  const targetX = playerShip.x + (clickX - width / 2);
  const targetY = playerShip.y + (clickY - height / 2);
...
// *****Section J12.1 - Code Reference Point – Finish******
...
// *****Section J19  - Code Reference Point – Finish******
  ...
  // ***** SECTION J23: UPDATE SHIP POSITION - Start ******
  ...
  // ***** SECTION J23: UPDATE SHIP POSITION - Finish ******
}
...
function render() {
  ...
  // Draw stars with parallax effect (your existing code)
  ctx.save();
...
//to view the location of the 
      ...
      document.getElementById('info').innerHTML =
	     `Location: (${trackPlayerLocation().x}, ${trackPlayerLocation().y})<br>` +
         `Target: ${laserTarget ? `(${laserTarget.x.toFixed(2)}, ${laserTarget.y.toFixed(2)})` : "Not Set"}`;
